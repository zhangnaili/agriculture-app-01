...
def load_chat_history(user1: str, user2: str) -> list:
    """加载用户间的聊天记录"""
    init_chat_db()
    try:
        ...
        all_chats = {}
    # 生成唯一对话ID（排序后拼接，确保双向一致）
    chat_id = "|".join(sorted([user1, user2]))
    return all_chats.get(chat_id, [])


def save_message(sender: str, receiver: str, content: str) -> None:
    """保存消息到聊天记录"""
    init_chat_db()
    try:
        with open(CHAT_DB, "r", encoding="utf-8") as f:
    ...
    chat_id = "|".join(sorted([sender, receiver]))
    message = {
        "sender": sender,
...
def chat_page():
    ...
    # 侧边栏：聊天对象选择（包含注册用户和系统角色）
    st.sidebar.subheader("选择聊天对象")

    # 1. 提取其他注册用户（排除当前用户）
    other_users = []
    registered_users = [uname for uname in users.keys() if uname != current_user]  # 纯用户名列表
    for uname in registered_users:
        # 简单状态模拟（实际项目可对接在线状态接口）
        status = "🟢 在线" if random.random() > 0.3 else "🔴 离线"  # 模拟70%在线率
        other_users.append(f"{uname} {status}")
    ...
    # 3. 合并聊天对象列表
    chat_partners = other_users + system_roles
    if not chat_partners:
        chat_partners = ["暂无可用聊天对象"]

    # 选择聊天对象（处理带状态标记的用户名）
    selected_partner_raw = st.sidebar.selectbox("聊天对象", chat_partners)
    # 提取纯用户名（去除状态标记）
    selected_partner = selected_partner_raw.split(" ")[0]
    ...
    with col_chat:
        # 显示当前聊天对象
        st.subheader(f"与 {selected_partner} 对话中")

        # 加载并显示聊天历史
        chat_history = load_chat_history(current_user, selected_partner)
        ...
        with chat_container:
            ...
            for msg in chat_history:
                ...
                if msg["sender"] == current_user:
                    st.markdown(
                        f"""
                        <div style="text-align: right; background-color: #e6f7ff; padding: 10px; 
                                 border-radius: 10px; margin: 5px 0; max-width: 85%; margin-left: auto;">
                            <strong>我</strong> 
                            <span style="color: #666; font-size: 0.8em; margin-left: 8px;">{msg['time']}</span><br>
                            {msg['content']}
                        </div>
                        """,
                ...
                # 对方发送的消息（左对齐，浅灰色背景）
                    ...
                    st.markdown(
                        f"""
                        <div style="text-align: left; background-color: #f0f2f6; padding: 10px; 
                                 border-radius: 10px; margin: 5px 0; max-width: 85%;">
        ...
        with st.form(key="chat_form", clear_on_submit=True):
            ...
            with btn_col2:
                refresh_btn = st.form_submit_button("刷新消息")

        # 发送消息逻辑
        if send_btn and msg_content.strip():
            if selected_partner not in ["暂无可用聊天对象"]:
                save_message(current_user, selected_partner, msg_content.strip())
                st.success("消息发送成功！")
                st.rerun()  # 替换为新版API
            else:
                st.warning("请选择有效的聊天对象")
    ...
    with col_side:
        ...
        for q in quick_questions:
            if st.button(q, use_container_width=True):
                save_message(current_user, selected_partner, q)
                st.rerun()  # 替换为新版API

        # 聊天状态（修复最后活跃显示逻辑）
        st.subheader("当前状态")
        if selected_partner in registered_users:  # 用纯用户名列表判断
            st.write(f"📅 最后活跃：刚刚")  # 实际项目可替换为用户最后登录时间
        st.write(f"💬 消息总数：{len(chat_history)}条")
